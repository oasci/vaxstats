...
% Compute Mean Squared Successive Difference (MSSD)
MSSD = SSSD / (BaseLength - 1);

% Calculate Rate-of-Change Thresholds
RateOfChangeUpperThreshold = 3 * sqrt(MSSD);
RateOfChangeLowerThreshold = -3 * sqrt(MSSD);

% Assign Rate-of-Change Thresholds to all data points
RateOfChangeUpper = repmat(RateOfChangeUpperThreshold, TotalLength, 1);
RateOfChangeLower = repmat(RateOfChangeLowerThreshold, TotalLength, 1);
```
...
# Calculate Mean Squared Successive Difference (MSSD)
MSSD <- SSSD / (length(Baseline) - 1)
